...
public class LoginServiceImpl implements LoginService{
	...
	public LoginResponse customerLogin(LoginDetails login){
		...
		try {
			...
			if(user.getStatus()) {
				flag = false;
				message = "Dear Customer"+user.getFname()+" your account has been blocked for security reasons";
			}
			if(!user.getAuthorizationStatus()) {
				flag = false;
				message = "Dear Customer"+user.getFname()+" your account has not been activated yet";
			}
